    ...
    if ($error === "") {
        ...
        if ($coverStmt) {
            ...
            if ($coverRow && !empty($coverRow["image_name"])) {
                ...
                $coverUpdate = $conn->prepare("UPDATE properties SET image=? WHERE id=?");
                if ($coverUpdate) {
                    $coverUpdate->bind_param("si", $newCover, $id);
                    $coverUpdate->execute();
                    $coverUpdate->close();
                }
            }
        ...
        if (!empty($changes)) {
            $logStmt = $conn->prepare("INSERT INTO property_change_log (property_id, changed_by_role, changed_by_user_id, field_name, old_value, new_value) VALUES (?, 'admin', NULL, ?, ?, ?)");
            if ($logStmt) {
                foreach ($changes as $change) {
                    [$field, $oldVal, $newVal] = $change;
                    $logStmt->bind_param("isss", $id, $field, $oldVal, $newVal);
                    $logStmt->execute();
                }
                $logStmt->close();
            }
