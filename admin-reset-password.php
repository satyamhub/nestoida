...
if ($email !== "" && $token !== "") {
    $resetId = reset_verify_token($conn, "admin", $email, $token);
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (!$validToken) {
        ...
        $updateAdmin->bind_param("ss", $passwordHash, $email);

        if ($updateAdmin->execute()) {
            reset_mark_used($conn, $resetId, "admin", $email);
...
<head>
    ...
    <title>Admin Reset Password - Nestoida</title>
    <script>
        (function () {
            try {
    ...
    <main class="max-w-xl mx-auto px-6 py-12">
        <div class="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm dark:bg-slate-900 dark:border-slate-800">
            <div class="flex items-center justify-between gap-2 mb-4">
                <h1 class="font-display text-3xl">Admin Reset Password</h1>
            ...
            <?php if ($validToken) { ?>
                <form method="POST" class="space-y-4">
                    ...
                    <button type="submit" class="w-full bg-slate-900 text-white py-3 rounded-xl font-semibold hover:bg-cyan-700 transition">Reset Admin Password</button>
                </form>
            <?php } elseif ($message === "") { ?>
            ...
            <div class="mt-4 flex gap-2 text-sm">
                ...
                <a href="admin-forgot-password.php" class="px-3 py-2 rounded-lg border border-slate-300 dark:border-slate-700">Forgot Password</a>
