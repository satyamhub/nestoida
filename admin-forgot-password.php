...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($email === "" || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    } elseif (reset_is_rate_limited($conn, "admin", $email, $requesterHash)) {
        ...
        if ($result && $result->num_rows > 0) {
            $token = reset_create_token($conn, "admin", $email, $requesterHash);
            if ($token !== null) {
                $resetLink = reset_base_url() . "/admin-reset-password.php?email=" . urlencode($email) . "&token=" . urlencode($token);
                if (!reset_send_email($email, $resetLink, "admin")) {
...
<head>
    ...
    <title>Admin Forgot Password - Nestoida</title>
    <script>
        (function () {
            try {
    ...
    <main class="max-w-xl mx-auto px-6 py-12">
        <div class="bg-white border border-slate-200 rounded-3xl p-8 shadow-sm dark:bg-slate-900 dark:border-slate-800">
            <div class="flex items-center justify-between gap-2 mb-4">
                <h1 class="font-display text-3xl">Admin Forgot Password</h1>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-300 mb-4">Enter admin email to receive a reset link.</p>
            ...
            <form method="POST" class="space-y-4">
                ...
                <button type="submit" class="w-full bg-slate-900 text-white py-3 rounded-xl font-semibold hover:bg-cyan-700 transition">Send Reset Link</button>
            </form>
